...
wk_rt = gs.get_worksheet(0)
wk_s = gs.get_worksheet(1)
wk_f = gs.get_worksheet(2)
...
def render_seaborn_chart():
	...
	conn = sqlite3.connect('sqlite3/intervention_times_sim.db')

	# Load the data from the SQLite database
	interventions_df = pd.read_sql_query("SELECT * from interventions", conn)

	# Filter the interventions_df to include only the desired ethnicity
	desired_ethnicity = 'Hispanic or Latino'
	interventions_df = interventions_df[interventions_df['Ethnicity'] == desired_ethnicity]
	...
	response_dimension = np.interp(interventions_df['ResponseTime'], scene_df['BeginTime'], scene_df[dimension])

	# Add the scatter plot on top of the line plot
	colors = {desired_ethnicity: 'black'}
	scatter = plt.scatter(interventions_df['ResponseTime'], response_dimension, c=interventions_df['Ethnicity'].map(colors), s=20, zorder=10)
...
def experience_menu():
	# Generate a random trainee_id
	trainee_id = random.randint(1000, 9999)  # Generate a random 4-digit number

	db = get_db()
	cur = db.cursor()

	# Check if trainee_id already exists in the database
	cur.execute('SELECT * FROM Trainees WHERE trainee_id = ?', (trainee_id,))
	existing_trainee = cur.fetchone()

	# If the trainee_id already exists, generate a new one
	while existing_trainee is not None:
		trainee_id = random.randint(1000, 9999)
		cur.execute('SELECT * FROM Trainees WHERE trainee_id = ?', (trainee_id,))
		existing_trainee = cur.fetchone()
	...
	# Insert the trainee_id, and session_id into the Session_Trainees table
	cur.execute('INSERT INTO Session_Trainees (session_id, trainee_id) VALUES (?, ?)', (session_id, trainee_id))
	db.commit()

	# Store the trainee_id in the session
