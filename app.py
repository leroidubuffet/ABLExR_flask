...
from utils import validate_session_id, map_ethnicity
from config import SECRET_KEY, PASSWORD
from constants import INVALID_PASSWORD, SESSION_ID_EXISTS, \
    SESSION_ID_MUST_BE_DIGIT, INSERT_FOUR_DIGIT_NUMBER, \
    SESSION_ID_NOT_EXIST, ETHNICITIES
...
def new_session():
    ...
    if request.method == 'POST':
        ...
        if ethnicity == 'random':
            ...
            ethnicity_id = map_ethnicity(ethnicity)
        ...
        if user_session_id and user_session_id.isdigit() and len(user_session_id) <= 3 and race_digit is not None:
            ...
            return render_template('new_session.html', ethnicities=ethnicities, 
                                   session_id=session_id, form_submitted=False,
                                   error=error)

    return render_template('new_session.html', ethnicities=ethnicities,
                           session_id=session_id, 
                           form_submitted=form_submitted, error=error)
